...
smooth.FEM.PDE.sv.basis<-function(locations = NULL, observations, FEMbasis, lambda, PDE_parameters, covariates = NULL, BC = NULL, GCV = FALSE, CPP_CODE = TRUE,GCVmethod = 2, nrealizations = 100)
{
 if(class(FEMbasis$mesh) == "MESH2D"){
  ...
  if(GCV == TRUE)
  {
    ...
    #_in bigsol[2] ho i dofs, che servono solo per il calcolo della GCV, sono (tr(S)+q)
  }else{
    reslist=list(fit.FEM=fit.FEM,PDEmisfit.FEM=PDEmisfit.FEM, beta = beta)
  }
...
getGCV<-function(locations, observations, fit.FEM, covariates = NULL, edf,ndim,mydim)
{
  ...
  for (i in 1:length(edf))  #_calcola di ogni lambda la GCV
  {
  ...
  return(list(stderr = stderr, GCV = GCV)) #_lista con std_error e con tutte le GCV, che poi ottimizzerà cercando il minimo tra i valori
}
