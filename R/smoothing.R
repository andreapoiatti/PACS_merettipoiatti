                     ...
                     BC = NULL, opt_strategy = 'no_batch', opt_method = 'GCV', stochastic = TRUE, lambdas = NULL, initial_lambda = NULL, nrealizations = 100)
{
  ...
  if(opt_method == 'GCV'){
    ...
    stop('No other method available outside GCV')
  }
  ...
  space_varying=checkSmoothingParameters(locations, observations, FEMbasis, covariates,
                                         incidence_matrix, BC, PDE_parameters, opt_strategy, opt_method,
                                         stochastic, lambdas, initial_lambda, nrealizations)
  ...
  if(!is.null(lambdas) && dim(lambdas)[1]==1 && dim(lambdas)[2]>=2)
    lambdas = t(lambdas)
  optm = as.matrix(optm)
  ...
  checkSmoothingParametersSize(locations, observations, FEMbasis, covariates, incidence_matrix,
                               BC, lambdas, space_varying, PDE_parameters, ndim, mydim)
  ...
  if(class(FEMbasis$mesh) == 'mesh.2D' & is.null(PDE_parameters)){	
    ...
    bigsol = CPP_smooth.FEM.basis(locations=locations, observations=observations, FEMbasis=FEMbasis, covariates=covariates,
                                  incidence_matrix=incidence_matrix, ndim=ndim, mydim=mydim, BC=BC, opt_method=optm,
                                  lambdas=lambdas, initial_lambda=initial_lambda, nrealizations=nrealizations)
    ...
    bigsol = NULL
    print('C++ Code Execution')
    bigsol = CPP_smooth.FEM.PDE.basis(locations=locations, observations=observations, FEMbasis=FEMbasis, PDE_parameters = PDE_parameters,
                                      covariates=covariates, incidence_matrix=incidence_matrix, ndim=ndim, mydim=mydim, BC=BC, opt_method=optm,
                                      lambdas=lambdas, initial_lambda=initial_lambda, nrealizations=nrealizations)
    ...
    bigsol = NULL
    print('C++ Code Execution')
    bigsol = CPP_smooth.FEM.PDE.sv.basis(locations=locations, observations=observations, FEMbasis=FEMbasis, PDE_parameters = PDE_parameters,
                                         covariates=covariates, incidence_matrix=incidence_matrix, ndim=ndim, mydim=mydim, BC=BC,opt_method=optm,
                                         lambdas=lambdas, initial_lambda=initial_lambda, nrealizations=nrealizations)
