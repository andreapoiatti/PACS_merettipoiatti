...
class MeshHandler<ORDER, 2, 2> {
	public:
		...
		*/
		MeshHandler(Real* points, UInt* edges, UInt* triangles, UInt* neighbors,
			    UInt num_nodes, UInt num_edges, UInt num_triangles):
			    points_(points), edges_(edges), elements_(triangles), neighbors_(neighbors),
			    num_nodes_(num_nodes), num_edges_(num_edges), num_elements_(num_triangles) {};
...
class MeshHandler<ORDER, 2, 3> {
	public:
		...
		MeshHandler(Real* points, UInt* triangles, UInt num_nodes, UInt num_triangles):
			    points_(points), elements_(triangles),
			    num_nodes_(num_nodes), num_elements_(num_triangles) {};
		...
		*/
		...
		{
			if (filename.find(".csv") != std::string::npos) {importfromCSV(filename);}
		}
...
class MeshHandler<ORDER, 3, 3> {
	public:
		...
		MeshHandler(Real* points, UInt* tetrahedrons, UInt num_nodes, UInt num_tetrahedrons):
		            points_(points), elements_(tetrahedrons),
			    num_nodes_(num_nodes), num_elements_(num_tetrahedrons) {};
