#include "../Headers/Solution_Builders.h"
...
SEXP build_sol(const VectorXr & solution, const output_Data & output, const MeshHandler<ORDER, mydim, ndim> & mesh , const InputHandler & regressionData )
{
      ...
      const VectorXi & elementIds = regressionData.getElementIds();
       ...
       SEXP result = NILSXP;
       result = PROTECT(Rf_allocVector(VECSXP, 23));
       ...
       UInt *rans2 = INTEGER(VECTOR_ELT(result, 5));
       rans2[0] = output.lambda_pos;
       ...
       rans = INTEGER(VECTOR_ELT(result, 7));
       rans[0] = output.n_it;
       ...
       SET_VECTOR_ELT(result, 14, Rf_allocVector(REALSXP, 1));
       rans = REAL(VECTOR_ELT(result, 14));
       rans[0] = output.time_partial;

       //! Copy betas
       SET_VECTOR_ELT(result, 15, Rf_allocMatrix(REALSXP, beta(0).size(), beta.size()));
       Real *rans4 = REAL(VECTOR_ELT(result, 15));
       ...
       SET_VECTOR_ELT(result, 16, Rf_allocVector(INTSXP, 1)); //tree_header information
       int *rans5 = INTEGER(VECTOR_ELT(result, 16));
       rans5[0] = mesh.getTree().gettreeheader().gettreelev();

       SET_VECTOR_ELT(result, 17, Rf_allocVector(REALSXP, ndim*2)); //tree_header domain origin
       Real *rans6 = REAL(VECTOR_ELT(result, 17));
       for(UInt i = 0; i < ndim*2; i++)
       	rans6[i] = mesh.getTree().gettreeheader().domainorig(i);

       SET_VECTOR_ELT(result, 18, Rf_allocVector(REALSXP, ndim*2)); //tree_header domain scale
       Real *rans7 = REAL(VECTOR_ELT(result, 18));
       for(UInt i = 0; i < ndim*2; i++)
       ...
       SET_VECTOR_ELT(result, 19, Rf_allocMatrix(INTSXP, num_tree_nodes, 3)); //treenode information
       int *rans8 = INTEGER(VECTOR_ELT(result, 19));
       ...
       SET_VECTOR_ELT(result, 20, Rf_allocMatrix(REALSXP, num_tree_nodes, ndim*2)); //treenode box coordinate
       Real *rans9 = REAL(VECTOR_ELT(result, 20));
       ...
       SET_VECTOR_ELT(result, 21, Rf_allocVector(INTSXP, elementIds.rows())); //element id of the locations point (vector)
       int *rans10 = INTEGER(VECTOR_ELT(result, 21));
       for(UInt i = 0; i < elementIds.rows(); i++)
       	rans10[i] = elementIds(i);

       SET_VECTOR_ELT(result, 22, Rf_allocMatrix(REALSXP, barycenters.rows(), barycenters.cols())); //barycenter information (matrix)
       Real *rans11 = REAL(VECTOR_ELT(result, 22));
