...
// TYPEDEFS
typedef UInt Id;      // Identification number, integer
typedef UInt BcId;    // Boundary condition identifier, integer
...
//   Rermark: this class inhrits form the Identifier
class Point: public Identifier
{
	public:
		UInt ndim;  // 2D or 3D point
	  ...
	  Point(Real x, Real y):
		...
		Point(Real x, Real y, Real z):
			Identifier(NVAL, NVAL)
			{coord_.resize(3); coord_[0]=x; coord_[1]=y; coord_[2]=z; ndim=3;}
		...
		// Access to coordinates operator
...
/*!
 * NNODES is the numbe of nodes used in the element
 *
 * mydim is the dimension of the object:
 * e.g. a triangle    has mydim = 2,
 *      a tethraedron has mydim = 3
 *
...
class Element<NNODES, 2, 2>: public Identifier
{
	...
	private:
		//std::array<Point, NNODES> points_;
		std::vector<Point>        points_;  // Vector of points of the element
...
// *** TETRAHEDRON ***
//! This class implements a Tetrahedron as an objects composed by four or ten nodes,
//! embedded in a 3-dimensional space. Currently, only the 4 nodes version is implemented.
...
inline Real evaluate_point(const Element<Nodes, mydim, ndim> & t,
	const Point & point, const Eigen::Matrix<Real, Nodes, 1> & coefficients)
{
...
//! 6 nodes triangle in 3D
template <>
inline Real evaluate_point<6, 2, 3>(const Element<6, 2, 3> & t,
	const Point & point, const Eigen::Matrix<Real, 6, 1> & coefficients)
{
	Eigen::Matrix<Real, 3, 1> bary_coeff = t.getBaryCoordinates(point);
	return ( 	coefficients[0]*(2*bary_coeff[0]*bary_coeff[0] - bary_coeff[0]) +
...
//! 4 nodes tetrahedron
template <>
inline Real evaluate_point<4, 3, 3>(const Element<4, 3, 3> & t,
	const Point & point, const Eigen::Matrix<Real, 4, 1> & coefficients)
{
...
//----------------------------------------------------------------------------//
template <UInt Nodes,UInt mydim, UInt ndim>
inline Eigen::Matrix<Real, ndim, 1> evaluate_der_point(const Element<Nodes, mydim, ndim> & t,
	const Point & point, const Eigen::Matrix<Real, Nodes, 1> & coefficients)
{
	//std::cerr<< "TRYING TO EVALUATE ORDER NOT IMPLEMENTED" << std::endl;
...
template <>
inline Eigen::Matrix<Real, 2, 1> evaluate_der_point<6, 2, 2>(const Element<6, 2, 2> & t,
	const Point & point, const Eigen::Matrix<Real, 6, 1> & coefficients)
{
...
template <>
inline Eigen::Matrix<Real, 3, 1> evaluate_der_point<4, 3, 3>(const Element<4, 3, 3> & t,
	const Point & point, const Eigen::Matrix<Real, 4, 1> & coefficients)
{
