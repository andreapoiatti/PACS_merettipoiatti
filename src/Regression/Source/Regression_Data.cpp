...
RegressionData::RegressionData(std::vector<Point> & locations, VectorXr & observations, UInt order, MatrixXr & covariates,
	...
	locations_(locations), observations_(observations), covariates_(covariates),
	order_(order), bc_values_(bc_values), bc_indices_(bc_indices), incidenceMatrix_(incidenceMatrix), arealDataAvg_(arealDataAvg),
...
RegressionData::RegressionData(std::vector<Point> & locations, std::vector<Real> & time_locations, VectorXr & observations, UInt order,
	...
	locations_(locations), time_locations_(time_locations), observations_(observations), covariates_(covariates),
	order_(order), bc_values_(bc_values), bc_indices_(bc_indices), incidenceMatrix_(incidenceMatrix), arealDataAvg_(arealDataAvg),
	flag_mass_(flag_mass), flag_parabolic_(flag_parabolic), ic_(ic), flag_SpaceTime_(true), search_(search)
{
	nRegions_ = incidenceMatrix_.rows();
	if(locations_.size()==0 && nRegions_==0)
	{
...
RegressionData::RegressionData(std::vector<Point> & locations, VectorXr & observations, UInt order, MatrixXr & covariates,
	...
	locations_(locations), observations_(observations), covariates_(covariates), WeightsMatrix_(WeightsMatrix),
	order_(order), bc_values_(bc_values), bc_indices_(bc_indices), incidenceMatrix_(incidenceMatrix), arealDataAvg_(arealDataAvg),
	flag_SpaceTime_(false), search_(search)
{
	nRegions_ = incidenceMatrix_.rows();
