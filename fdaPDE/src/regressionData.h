...
//! A basic version of the constructor.
		...
		*/
		...
		inline const VectorXr * getWeightsMatrix() const {return &WeightsMatrix_;}
		...
		inline const std::vector<UInt> * getObservationsNA() const {return &observations_na_;}

		//! A method returning the space penalization term
		inline const std::vector<Real> * getLambdaS() const {return &lambdaS_;}
		//! A method returning the the time penalization term
		inline const std::vector<Real> * getLambdaT() const {return &lambdaT_;}
		...
		inline const std::vector<UInt> *  getDirichletIndices() const {return &bc_indices_;}
		//! A method returning the values to apply for Dirichlet Conditions
		inline const std::vector<Real> *  getDirichletValues() const {return &bc_values_;}
		//! A method returning the values to apply for Initial Conditions
		inline const VectorXr * getInitialValues() const {return &ic_;}
		...
		inline MatrixXr const & getDOF_matrix() const {return dof_matrix_;} //mot pointer to avoid compilation error in templates, not used in mixedFERegression

		inline MatrixXr const & getBarycenters() const {return barycenters_;} //mot pointer to avoid compilation error in templates, not used in mixedFERegression
		inline VectorXi const & getElementIds() const {return element_ids_;} //mot pointer to avoid compilation error in templates, not used in mixedFERegression
